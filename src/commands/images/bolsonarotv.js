  ...
  .setDescription(`**Por favor ${message.author}, espere **\`${time}\`** para executar outro comando**`)
    ...
    msg.delete({ timeout: 600000 });
       ...
       return;
        ...
        let img = jimp.read("https://cdn.discordapp.com/attachments/821095284443447357/822148976694001674/bolsonarotv.png")
        if (!args[0]) return message.reply("Escreva algo para o bolsonaro falar.")
                        img.then(image => {
                          jimp.loadFont(jimp.FONT_SANS_32_BLACK).then(font => {
                       image.resize(885, 494)
                                           image.print(font, 500, 150, args.join(" "), 7000)
                image.getBuffer(jimp.MIME_PNG, (err, i) => {
                    message.channel.send({files: [{ attachment: i, name: "bolsonarotv.png"}]})
                })
            })
        })
    }